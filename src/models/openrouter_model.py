...
class OpenRouterModel(BaseMultiKeyModel):
    """
    Tier 3 — Cloud frontier model.
    Runs Nous Hermes 3 405B (Llama 3.1 405B) via OpenRouter API (OpenAI-compatible endpoint).
    Free tier model: nousresearch/hermes-3-llama-3.1-405b:free

    Supports API key rotation across multiple OpenRouter accounts.
    Published pricing (Meta/Nous): $2.66/1M input, $2.66/1M output.
    """

    def __init__(
        self,
        model_id: str = "nousresearch/hermes-3-llama-3.1-405b:free",
        ...
        num_tokens = len(keys)
        delay = min_delay if min_delay is not None else max(1.0, 60.0 / (num_tokens * 10))
        
        print(f"    OpenRouter: {num_tokens} token(s), {delay:.1f}s delay between calls")
        
        super().__init__("OpenRouter", keys, delay)
        self._init_client()
        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                if classification == "PERMANENT_EXHAUSTION":
                    ...
                    print(f"    ⏳ OpenRouter rate limited (transient), rotated key, waiting {backoff}s...")
                    time.sleep(backoff)
                    continue

        latency = time.perf_counter() - start
        return ModelResponse(
            text=f"[ERROR] OpenRouter call failed: All keys exhausted or server error",
