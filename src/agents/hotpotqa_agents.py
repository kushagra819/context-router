"""
HotpotQA 3-Agent Pipeline: Analyzer → Solver → Verifier
...
ANALYZER_USER = """Given a multi-hop Question and a set of Context Documents, perform the following tasks:
1. Identify the sub-questions (reasoning hops) required to answer the question.
2. Select only the relevant facts, sentences, and documents from the context that are needed for each hop.
...
SOLVER_USER = """Solve this multi-hop question using the provided context documents and the step-by-step plan/facts identified by the analyzer.
...
Ensure that:
1. All steps in the reasoning are fully supported by the context documents.
...
def create_hotpotqa_agents(model: BaseModel) -> dict[str, BaseAgent]:
    """Create the 3 HotpotQA agents all using the same model."""
    return {
        "analyzer": BaseAgent(
            role="analyzer",
...
def format_hotpotqa_context(context_dict: dict) -> str:
    """Format the context dict from HotpotQA dataset into a clean string for prompts."""
    formatted_context = ""
...
def run_hotpotqa_pipeline(agents: dict[str, BaseAgent], question: str, context_dict: dict) -> dict:
    """
    Run the full 3-agent pipeline on a single HotpotQA problem.
    ...
    formatted_context = format_hotpotqa_context(context_dict)
