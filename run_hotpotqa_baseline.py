"""
Run HotpotQA baseline experiments — all agents use the SAME model (no routing).

Usage:
    python run_hotpotqa_baseline.py --tier 1 --num-problems 200
    python run_hotpotqa_baseline.py --tier 2 --num-problems 200
    python run_hotpotqa_baseline.py --tier 3 --num-problems 50 --rate-limit 2
    python run_hotpotqa_baseline.py --tier 4 --num-problems 200 --resume
"""
...
from src.agents.hotpotqa_agents import create_hotpotqa_agents, run_hotpotqa_pipeline
...
def run_hotpotqa_baseline(tier: int, num_problems: int, rate_limit_rpm: int | None = None, resume: bool = False):
    """Run HotpotQA baseline with all agents using the specified tier."""

    experiment_id = f"hotpotqa_baseline_tier{tier}"
    model_name = MODEL_CONFIG[tier]["name"]

    print(f"\n{'='*60}")
    print(f"  HotpotQA Baseline — Tier {tier} ({model_name})")
    ...
    # Load dataset
    print("Loading HotpotQA dataset (validation split, distractor setting)...")
    ...
    agents = create_hotpotqa_agents(model)
    ...
    for i, item in enumerate(tqdm(problems, desc=f"Tier {tier}")):
        ...
        try:
            ...
            result = run_hotpotqa_pipeline(agents, question_text, context_dict)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num-problems", type=int, default=200,
                        help="Number of HotpotQA problems to evaluate (default: 200)")
    ...
    validation_problems = list(load_dataset("hotpotqa/hotpot_qa", "distractor", split="validation"))[:args.num_problems]

    run_hotpotqa_baseline(
        tier=args.tier,
